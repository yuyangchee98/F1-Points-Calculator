...
// Async thunk for fetching season data (schedule + results + teams)
export const fetchSeasonData = createAsyncThunk(
  'races/fetchSeasonData',
  async (year: number) => {
    const [schedule, results, teams] = await Promise.all([
    ...
    // Teams need to be dispatched to teamsSlice separately since this is racesSlice
    // We'll return teams in the payload for teamsSlice to handle
    return { schedule, results, teams };
  }
);
